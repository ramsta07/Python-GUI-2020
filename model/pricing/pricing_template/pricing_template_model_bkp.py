...
class Pricing_Template_Model():
		...
		for usage_type in l_usage_type:
			...
			for r, d, f in os.walk(folder_path):
				for file in f:
					...
					if usage_type.lower() in file.lower():
	...
	def read_source_to_DF(self, keyword, folder, pricing_tab):
		...
		df = pd.read_csv(src_filename, sep, engine='python')
	...
	def get_template_mappings(self, usage_type, template_sheet, df, pricing_tab):
		...
		rows = template_sheet.iter_rows(min_row=2, max_row=2)
		first_row = next(rows)
		template_columns = [(c.value, c.coordinate) for c in first_row]
			...
			if temp_column.lower() in [' '.join(col.lower().split('_')[1:]) for col in src_columns]:
				...
				for src_col in src_columns:
					...
					if temp_column.lower() == column_drv:
						...
						d_mapping = {'Usage_Type': usage_type, 'Sheet': template_sheet, 'Source_Column': src_column, 'Template_Column': temp_column, 'Template_Cell' : temp_cell}

			else:

				l_nomapping.append({'Sheet': template_sheet, 'Template_Column': temp_column, 'Template_Cell' : temp_cell})

			if d_mapping: l_mapping.append(d_mapping)

		# Update templates OD 

		key_tab = pricing_tab + '-' + template_sheet.title
	...
	def prepare_worksheet(self, sheet, start_row, max_row):
		...
		copy_low_range = 'A' + str(start_row)
		copy_high_range = get_column_letter(sheet.max_column)+str(start_row)
		...
		if max_row == 0:
			...
			for a in sheet[copy_low_range:copy_high_range]:
				for cell in a:
					colNum = cell.col_idx
					value = cell.value
					
					for rowNum in range(start_row,start_row+max_row):
	...
	def populate_worksheet(self, sheet, df, l_mapping, start_row):
		...
		for d in l_mapping:
			...
			for data in l_values:

				sheet[temp_column_cell + str(startingrow)].value = data

				startingrow = startingrow + 1

		row_cnt = startingrow - start_row
